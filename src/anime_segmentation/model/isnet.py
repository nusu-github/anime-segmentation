...
def muti_loss_fusion(preds, target):
    ...
    for i in range(len(preds)):
        if preds[i].shape[2] != target.shape[2] or preds[i].shape[3] != target.shape[3]:
            ...
            loss += bce_loss(preds[i], tmp_target)
        else:
...
def muti_loss_fusion_kl(preds, target, dfs, fs, mode="MSE"):
    ...
    for i in range(len(dfs)):
        ...
        if mode == "MSE":
            ...
            loss += kl_loss(F.log_softmax(df, dim=1), F.softmax(fs_i, dim=1))
        elif mode == "MAE":
            loss += l1_loss(df, fs_i)
        elif mode == "SmoothL1":
            loss += smooth_l1_loss(df, fs_i)

    return loss0, loss


class REBNCONV(nn.Module):
    def __init__(self, in_ch=3, out_ch=3, dirate=1, stride=1) -> None:
        super().__init__()

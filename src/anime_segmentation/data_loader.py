...
class RescalePad:
    ...
    def __call__(self, sample):
        ...
        if h != self.output_size or w != self.output_size:
            ...
            image = TF.pad(image, [pw // 2, ph // 2, pw // 2 + pw % 2, ph // 2 + ph % 2], 0)
            label = TF.resize(label, [new_h, new_w])
            label = TF.pad(label, [pw // 2, ph // 2, pw // 2 + pw % 2, ph // 2 + ph % 2], 0)
...
class GaussianNoise:
    def __init__(self, mean: int = 0, sigma: float = 0.05) -> None:
        self.mean = mean
        self.sigma = sigma
...
class AnimeSegDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        if (
            self.use_cache
            and self.cache_idx[idx] != -1
            and self.cache_use_count[idx] != 0
            and self.cache_use_count[idx] < self.cache_life
        ):
        ...
        if idx < len(self.real_img_list):
            ...
            label = cv2.imread(str(self.real_mask_list[idx]), cv2.IMREAD_GRAYSCALE)[
                :, :, np.newaxis
            ]
...
def create_training_datasets(
...
) -> tuple[AnimeSegDataset, AnimeSegDataset]:
    data_root = Path(data_root)
    imgs_dir = data_root / imgs_dir
    ...
    transform = transforms.Compose(
        [
            RescalePad(image_size + image_size // 4),
            RandomRot(),
    ...
    train_generator = DatasetGenerator(
        train_bg_list, train_fg_list, (image_size, image_size), (image_size, image_size)
    )
    train_dataset = AnimeSegDataset(
        ...
        cache_life=cache_update_epoch,
    )
    val_generator = DatasetGenerator(
        val_bg_list, val_fg_list, (image_size, image_size), (image_size, image_size)
    )

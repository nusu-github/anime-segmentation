...
class AnimeSegDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if idx < len(self.real_img_list):
            image = cv2.cvtColor(cv2.imread(str(self.real_img_list[idx]), cv2.IMREAD_COLOR), cv2.COLOR_BGR2RGB)
            label = cv2.imread(str(self.real_mask_list[idx]), cv2.IMREAD_GRAYSCALE)[:, :, np.newaxis]
...
def create_training_datasets(data_root, fgs_dir, bgs_dir, imgs_dir, masks_dir, fg_ext, bg_ext, img_ext, mask_ext,
    ...
    train_img_list = sorted(list(imgs_dir.glob('*' + img_ext)))
    train_mask_list = []
    for img_path in train_img_list:
        train_mask_list.append(masks_dir / img_path.name.replace(img_ext, mask_ext))
    
    train_fg_list = sorted(list(fgs_dir.glob('*' + fg_ext)))
    train_bg_list = sorted(list(bgs_dir.glob('*' + bg_ext)))

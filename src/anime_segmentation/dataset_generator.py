...
class DatasetGenerator:
    ...
    def __init__(self, bg_list, fg_list, output_size_range_h=(512, 1024), output_size_range_w=(512, 1024),
        ...
        if load_all:
            ...
            for fg_path in tqdm(fg_list):
                fg = cv2.cvtColor(cv2.imread(str(fg_path), cv2.IMREAD_UNCHANGED), cv2.COLOR_BGRA2RGBA)
                assert fg.shape[2] == 4
                self.fgs.append(fg)
    ...
    def __getitem__(self, idx):
        ...
        if self.load_all:
            ...
            fgs = [cv2.cvtColor(cv2.imread(str(self.fg_list[x]), cv2.IMREAD_UNCHANGED), cv2.COLOR_BGRA2RGBA).astype(
                np.float32) / 255 for x in self.characters_idx[idx]]
            bg = cv2.cvtColor(cv2.imread(str(self.bg_list[bg_idx]), cv2.IMREAD_COLOR), cv2.COLOR_BGR2RGB).astype(
                np.float32) / 255
        ...
        if aug and self.random.randint(0, 1) == 0:
            ...
            for _ in range(0, self.random.randint(1, 10)):
                if len(self.fonts) == 0:
                    ...
                    self.fonts = [ImageFont.truetype(font_path, x, encoding="utf-8") for x in range(s, s * 5, 2)]

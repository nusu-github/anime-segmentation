...
def vector_included_angle(v1, v2) -> float:
...
class DatasetGenerator:
    def __init__(
        ...
        bg_list,
        fg_list,
        output_size_range_h: tuple[int, int] = (512, 1024),
        ...
        if load_all:
            ...
            for bg_path in tqdm(bg_list):
                bg = cv2.cvtColor(cv2.imread(str(bg_path), cv2.IMREAD_COLOR), cv2.COLOR_BGR2RGB)
                self.bgs.append(bg)
            print("loading fgs")
            for fg_path in tqdm(fg_list):
                fg = cv2.cvtColor(
                    cv2.imread(str(fg_path), cv2.IMREAD_UNCHANGED), cv2.COLOR_BGRA2RGBA
                )
                assert fg.shape[2] == 4
                self.fgs.append(fg)
    ...
    def process_fg(self, fg, output_size: list[int], scale: float):
        assert fg.shape[2] == 4
        h, w = fg.shape[:2]
    ...
    def simulate_light(self, image, strength=0.2):
        ...
        rs = [
            vector_included_angle([-cx, -cy], c_v),
        ...
        if self.random.randint(0, 1) == 0:
            light_mask = np.full(
                [*img_size, 3], (1 + strength, 1 + strength, 1 + strength), dtype=np.float32
            )
        else:
    ...
    def __getitem__(self, idx):
        ...
        if self.load_all:
            ...
            fgs = [
                cv2.cvtColor(
                    cv2.imread(str(self.fg_list[x]), cv2.IMREAD_UNCHANGED), cv2.COLOR_BGRA2RGBA
                ).astype(np.float32)
                / 255
            ...
            bg = (
                cv2.cvtColor(
                    cv2.imread(str(self.bg_list[bg_idx]), cv2.IMREAD_COLOR), cv2.COLOR_BGR2RGB
                ).astype(np.float32)
                / 255
            )
        ...
        if aug and self.random.randint(0, 1) == 0:
            ...
            bg_mask = cv2.drawContours(
                np.zeros([*output_size, 1], dtype=np.float32), counts, 0, (1.0,), cv2.FILLED
            )
            ...
            if self.random.randint(0, 1) == 0:
                ...
                bg = cv2.drawContours(
                    bg, counts, 0, edge_color, self.random.randint(ms // 600, ms // 400)
                )
        ...
        if aug and self.random.randint(0, 1) == 0 and len(fgs) == 1 and not small:
            ...
            image_edge = image_edge * (
                1 - (label - cv2.erode(label, np.ones([3, 3]))[:, :, np.newaxis])
            )
            if t == 0:
                if self.random.randint(0, 1) == 0:
        ...
        if aug and self.random.randint(0, 1) == 0:
            ...
            trans_mat = cv2.getRotationMatrix2D(
                (output_size[1] // 2, output_size[0] // 2), angle, 1
            )
        ...
        if aug and self.random.randint(0, 1) == 0:
            ...
            for _ in range(self.random.randint(1, 10)):
                if len(self.fonts) == 0:
                    s = min(h, w) // 100
                    font_path = str(Path(__file__).parent / "assets" / "font.otf")
                    self.fonts = [
                        ImageFont.truetype(font_path, x, encoding="utf-8")
                        for x in range(s, s * 5, 2)
                    ]
        ...
        if aug:
            ...
            image = cv2.warpAffine(
                image, rot, (w, h), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT
            )
